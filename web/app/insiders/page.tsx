...
import { q, type InsiderTrade } from "@/lib/db";
import { fmtCap, fmtDate, fmtNum } from "@/lib/format";
import { Panel, SectionTitle, Empty } from "@/components/ui";

export const revalidate = 300;
export const metadata = { title: "Insider İşlemleri" };

type Row = InsiderTrade & { stocks: { name: string } | null };

export default async function InsidersPage() {
  const [buys, sells] = await Promise.all([
    q<Row>("insider_trades", "select=*,stocks(name)&transaction_code=eq.P&order=filing_date.desc&limit=60", 300),
    q<Row>("insider_trades", "select=*,stocks(name)&transaction_code=eq.S&order=filing_date.desc&limit=30", 300),
  ]);
