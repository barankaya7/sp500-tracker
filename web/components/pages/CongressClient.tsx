...
import { type CongressTrade } from "@/lib/db";
...
export default function CongressClient() {
  const { data: trades, loading } = useData(() =>
    cq<Row>("congress_trades", "select=*,stocks(name)&order=disclosure_date.desc&limit=150")
  );
  if (loading || !trades) return <Skeleton rows={3} />;
