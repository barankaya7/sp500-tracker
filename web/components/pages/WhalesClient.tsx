...
import { type WhaleFund, type WhaleHolding } from "@/lib/db";
...
export default function WhalesClient() {
  const { data, loading } = useData(async () => {
    const [funds, holdings] = await Promise.all([
      cq<WhaleFund>("whale_funds", "select=*&order=name"),
      cq<WhaleHolding>("whale_holdings", "select=*&order=value_usd.desc.nullslast&limit=2000"),
    ]);
    return { funds, holdings };
  });
  if (loading || !data) return <Skeleton rows={3} />;
  const { funds, holdings } = data;
