...
import type {
  Stock, Quote, PriceBar, Fundamentals, Score,
  InsiderTrade, WhaleHolding, CongressTrade, NewsItem, Earnings,
} from "@/lib/db";
...
type WH = WhaleHolding & { whale_funds: { name: string; manager: string | null } | null };
...
export default function StockClient({ symbol }: { symbol: string }) {
  const { data, loading } = useData(async () => {
    ...
    const [stockArr, quoteArr, bars, fundArr, scoreArr, insiders, whales, congress, news, earnArr] = await Promise.all([
      cq<Stock>("stocks", `select=*&symbol=eq.${symbol}`),
      cq<Quote>("quotes_latest", `select=*&symbol=eq.${symbol}`),
      cq<PriceBar>("prices_daily", `select=date,open,high,low,close,volume&symbol=eq.${symbol}&order=date&limit=400`),
      cq<Fundamentals>("fundamentals", `select=*&symbol=eq.${symbol}`),
      cq<Score>("scores_daily", `select=*&symbol=eq.${symbol}&order=date.desc&limit=1`),
      cq<InsiderTrade>("insider_trades", `select=*&symbol=eq.${symbol}&order=filing_date.desc&limit=15`),
      cq<WH>("whale_holdings", `select=*,whale_funds(name,manager)&symbol=eq.${symbol}&order=value_usd.desc&limit=15`),
      cq<CongressTrade>("congress_trades", `select=*&symbol=eq.${symbol}&order=transaction_date.desc&limit=10`),
      cq<NewsItem>("news", `select=*&symbol=eq.${symbol}&order=published_at.desc&limit=12`),
      cq<Earnings>("earnings_calendar", `select=*&symbol=eq.${symbol}&earnings_date=gte.${today}&order=earnings_date&limit=1`),
    ]);
    return { stockArr, quoteArr, bars, fundArr, scoreArr, insiders, whales, congress, news, earnArr };
  }, [symbol]);

  if (loading || !data) return <Skeleton rows={4} />;
  const { stockArr, quoteArr, bars, fundArr, scoreArr, insiders, whales, congress, news, earnArr } = data;

  const stock = stockArr[0];
  if (!stock) return <Empty text={`${symbol} bulunamadı.`} />;
